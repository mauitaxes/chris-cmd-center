...
- **Task 3c-proxy — Calendar serverless proxy [deferred; decided 2026-06-08]:** 3c shipped **MCP-only** (commit `dc904e4`) — the Schedule lane populates via the Google Calendar MCP in Cowork and renders a graceful empty state on the deployed Netlify site, which has **no** calendar proxy. To make the lane populate in the deployed browser, build `netlify/functions/calendar-proxy.js`: Google OAuth creds server-side, injectable-`fetchImpl` test pattern, mirror `notion-proxy.js`/`todoist-proxy.js`, emit the same `{events:[{start,end,summary,htmlLink,status}]}` shape `loadCalendar()` already parses, and route `list_events`/`list_calendars` through `call()` by the calendar MCP uuid the way `todoist-proxy` routes Todoist. No client change needed beyond the `call()` route. Read-only. Gate: needs Google API credentials in Netlify env (a Chris-input gate, like G1 for Todoist).
...
- **Worktree first move (every session):** run `bash scripts/cc-worktree-bootstrap.sh` from the `feat/todoist-phase1` worktree before any git op. The worktree pointers are stored **relative** (mount-independent) so they no longer go stale across Cowork remounts; the script re-asserts them and clears stale `*.lock` files. Never hand-edit the pointers, and never `rm` a lock — the drvfs mount blocks `unlink` ("Operation not permitted"); use `mv` (the script does). The sandbox git is 2.34.1 and separate from your machine git — a machine git upgrade changes nothing here and is not needed (relative pointers already work on 2.34).
