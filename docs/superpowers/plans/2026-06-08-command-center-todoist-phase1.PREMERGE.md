...
- **Task 4 — Recurring-completion spike [S] → CP3 gate: ✅ DONE 2026-06-08, PASS** (`TASK4-SPIKE-RESULT.md`; reconciled with branch spike `f63654e`): numerator source = **`find-activity` (eventType=completed)**, NOT `find-completed-tasks` (which misses advancing recurrences); scoping **exact-project, non-recursive** (fetch workspace-wide, filter client-side by 7 sub-project IDs); `eventDate` UTC → window in HST; `hstDayUtcWindow`+`completedInTreeOnDay` already landed on branch. Recurrence-survival RESOLVED: plain `"every day"` advances/survives; `"...starting today"` closes the series — migration/quick-add must use plain phrasing.
...
- **CP3: ✅ CLEARED 2026-06-08** — recurring-completion spike (Task 4) PASSED; numerator source confirmed = `find-activity` (not `find-completed-tasks`), scoping + TZ + recurrence-survival all captured in `TASK4-SPIKE-RESULT.md`. Still hold the streak *flip* until Task 7 (nightly job) lands.
